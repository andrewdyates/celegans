...
collapse.cls <- function(CLS, idx) {
  ...
  R$COH <- matrix(0.0, nrow=n, ncol=n)
  ...
  for (i in 1:length(levels(idx))) {
    for (j in 1:length(levels(idx))) {
      ...
      #print(paste(i,j,sum(iv),sum(jv),sep=", "))
      #print(C)
      #print(r)
      if (is.null(r$cls)) {
        print(C)
...
get.scores <- function(coh, size) {
  nn <- sum(size)
  p <- size/nn
  ln.p <- log(p,base=2)
  p.ln.p <- -p*ln.p
  p.ln.p[is.na(p.ln.p)] <- 0
  R <- list()
  R$wavg <- sum(p*coh)
  R$hwavg <- sum(p.ln.p*coh)
  R
}

get.coh.M.score <- function(COLLAPSED) {
  R <- list()
  tri <- upper.tri(COLLAPSED$COH,diag=F)
  diag <- upper.tri(COLLAPSED$COH,diag=T) & !tri
  R$tri <- get.scores(COLLAPSED$COH[tri], COLLAPSED$SIZE[tri])
  R$diag <- get.scores(COLLAPSED$COH[diag], COLLAPSED$SIZE[diag])
  R
}  
...
CLS <- D.expr.gold$CLS
#idx <- cutree(H,k=7)
#R <- collapse.cls(CLS, idx)
#get.coh.M.score(R)


get.compression <- function(CLS, H) {
  n <- round(dim(CLS)[1]/2)+2
  R <- list()
  R$e.t <- rep(0,n); R$e.c <- rep(0,n)
  R$w.t <- rep(0,n); R$w.c <- rep(0,n)
  for(k in 1:n) {
    print(k)
    idx <- cutree(H,k)
    C <- collapse.cls(CLS,idx)
    S <- get.coh.M.score(C)
    R$e.t[k] <- S$tri$hwavg
    R$w.t[k] <- S$tri$wavg
    R$e.c[k] <- S$diag$hwavg
    R$w.c[k] <- S$diag$wavg
  }
  R
}

Z <- get.compression(CLS,H)

## H <- as.hclust(TRANS.R$Rhclust)
## CLS <- D.expr.trans$CLS
## ZZ <- get.compression(CLS, H)
